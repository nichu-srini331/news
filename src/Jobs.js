import React, { useEffect,useState } from 'react'
import Navbar from './Navbar'

function Jobs() {
  const[latest,setlatest] = useState([])
   
  useEffect(()=>{
get()
},[])

    const get = () => {
      fetch("https://api.adzuna.com/v1/api/jobs/in/search/1?app_id=35b50d85&app_key=22b36c8c1f5d8b5b6fff09d526f4b98b").then((res)=>res.json()).then((j)=>setlatest(j.results))
       console.log(latest)
        }
        
  return (
    <div className='container-fluid'>
      <div className='row'>
        <div className='col-2'>
          <Navbar/>
        </div>
        <div className='col-10'>
          <div className='row ser'>
          <div className='col-3'><h1 className='h1-trend'>Job Hunting</h1>
        </div>
            <div className='col-9'>
            <input class="form-control ser-in" type="search" placeholder="Search" aria-label="Search"/>
      <button class="btn btn-dash" type="submit">Search</button></div>
          </div>
          <div className='row'>
          <div class="row">
            {latest.map(lat=>{return(
              
              <div class="col-sm-4">
                <div class="card">
                  <div class="card-body">
                    <h5 class="card-title">{lat.title}</h5>
                    <p class="card-text">{lat.description}</p>
                    <a href={lat.link} class="btn btn-primary">Explore more</a>
                  </div>
                </div>
              </div>
              
            
            )})
            }
          </div>
          </div>
        </div>

      </div>
      



    </div>
  )
}

export default Jobs